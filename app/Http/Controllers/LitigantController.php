...
class LitigantController extends Controller
{
    ...
    public function searchLitigants(Request $request)
    {
        ...
        // Sửa logic tìm kiếm - gom tất cả điều kiện trong một where function
        $litigants = Litigant::with(['individualLitigant.identityDocuments'])
            ->where(function ($queryBuilder) use ($query) {
                // Tìm theo tên
                $queryBuilder->where('full_name', 'like', '%' . $query . '%');

                // Hoặc tìm theo số giấy tờ tùy thân (chỉ áp dụng cho cá nhân)
                $queryBuilder->orWhere(function ($subQuery) use ($query) {
                    $subQuery->where('type', 'individual')
                        ->whereHas('individualLitigant.identityDocuments', function ($docQuery) use ($query) {
                            $docQuery->where('document_number', 'like', '%' . $query . '%');
                        });
                });
            })
            ->limit(10)
