...
class AssetController extends Controller
{
    ...
    public function apiSearch(Request $request): JsonResponse
    {
        try {
            ...
            // Debug logging
            Log::info('AssetController::apiSearch called', [
                'query' => $query,
            ...
            // Tìm kiếm đơn giản theo cột name (đã được generate sẵn) và notes
            $assets = Asset::where(function ($q) use ($query) {
                // Tìm theo name (chính)
                $q->where('name', 'like', "%{$query}%")
                    // Hoặc tìm theo notes (phụ)
            ...
            Log::info('Found assets', [
                'count' => $assets->count(),
                'sample_names' => $assets->take(3)->pluck('name')->toArray()
            ]);

            $result = $assets->map(function ($asset) {
                return [
                    'id' => $asset->id,
                    'text' => $asset->name ?: "Tài sản #{$asset->id}", // Dùng name đã generate
                    'asset_type' => $asset->asset_type,
                    'type_label' => $this->getAssetTypeLabel($asset->asset_type),
                    'notes' => $asset->notes,
                    'url' => route('properties.show', $asset),
                ];
            });
            ...
            Log::info('Formatted result', [
                'count' => $result->count(),
                'sample_data' => $result->take(2)->toArray()
            ]);
            ...
            return response()->json([
                'error' => 'Có lỗi xảy ra khi tìm kiếm tài sản: ' . $e->getMessage()
            ], 500);
        }
    }
    ...
    /**
     * Get asset type label - cải tiến để match với constants trong model
     */
    private function getAssetTypeLabel($assetType)
    {
        $labels = [
            Asset::TYPE_REAL_ESTATE_HOUSE => 'Nhà ở',
            Asset::TYPE_REAL_ESTATE_APARTMENT => 'Căn hộ',
            Asset::TYPE_REAL_ESTATE_LAND_ONLY => 'Đất',
            Asset::TYPE_MOVABLE_PROPERTY_CAR => 'Xe ô tô',
            Asset::TYPE_MOVABLE_PROPERTY_MOTORCYCLE => 'Xe mô tô',

            // Backward compatibility với enum cũ nếu có
            'land_plot' => 'Đất',
            'house' => 'Nhà ở',
            'apartment' => 'Căn hộ',
            'vehicle' => 'Xe',
        ...
        return $labels[$assetType] ?? 'Không xác định';
    }

    /**
     * Method helper để đảm bảo tất cả assets có name
     * Gọi khi cần thiết để generate name cho assets cũ
     */
    public function generateMissingAssetNames()
    {
        try {
            $assetsWithoutName = Asset::whereNull('name')
                ->orWhere('name', '')
                ->with(['certificates', 'landPlots', 'vehicle'])
                ->get();

            $updated = 0;
            foreach ($assetsWithoutName as $asset) {
                $asset->generateAndUpdateName();
                $updated++;
            }

            Log::info("Generated names for {$updated} assets");

            return response()->json([
                'success' => true,
                'message' => "Đã tạo tên cho {$updated} tài sản",
                'updated_count' => $updated
            ]);

        } catch (\Exception $e) {
            Log::error('Error generating asset names: ' . $e->getMessage());

            return response()->json([
                'error' => 'Có lỗi xảy ra khi tạo tên tài sản: ' . $e->getMessage()
            ], 500);
        }
    }
}
