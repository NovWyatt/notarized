    ...
    <div class="container-fluid p-3">
        ...
        <nav aria-label="breadcrumb">
            <ol class="breadcrumb">
                ...
                <li class="breadcrumb-item"><a href="{{ route('admin.dossiers.show', $dossier) }}">{{ $dossier->name }}</a>
                </li>
                    ...
                    <ul class="dropdown-menu">
                        <li>
                            <a class="dropdown-item"
                                href="{{ route('admin.dossiers.contracts.export.pdf', [$dossier, $contract]) }}">
                                <i class="fas fa-file-pdf me-2"></i>Xuất PDF
                            </a>
                        </li>
                        <li>
                            <a class="dropdown-item"
                                href="{{ route('admin.dossiers.contracts.export.word', [$dossier, $contract]) }}">
                                                ...
                                                <div class="card bg-light border-0">
                                                    <div class="card-body py-2">
                                                        <div class="d-flex align-items-start">
                                                            <div
                                                                class="avatar-sm bg-primary bg-opacity-10 rounded-circle me-3 d-flex align-items-center justify-content-center">
                                                            ...
                                                            <div class="flex-grow-1">
                                                                ...
                                                                <div class="small text-muted">
                                                                    ...
                                                                    @if($party->litigant->type === 'individual' && $party->litigant->individualLitigant)
                                                                        <div>{{ $party->litigant->identity_info }}</div>
                                                                        @php
                                                                            $address = $party->litigant->addresses->where('address_type', 'permanent')->first();
                                                                        @endphp
                                                                        @if($address)
                                                                            <div>ĐC: {{ $address->full_address }}</div>
                                                                        @endif
                                                                    @elseif($party->litigant->type === 'organization' && $party->litigant->organization)
                                                                        <div>MST: {{ $party->litigant->organization->tax_code }}</div>
                                                                        @php
                                                                            $address = $party->litigant->addresses->first();
                                                                        @endphp
                                                                        @if($address)
                                                                            <div>ĐC: {{ $address->full_address }}</div>
                                                                        @endif
                                                                    @elseif($party->litigant->type === 'credit_institution' && $party->litigant->creditInstitution)
                                                                        <div>Loại hình:
                                                                            {{ $party->litigant->creditInstitution->organization_type }}
                                                                        </div>
                                                                        @php
                                                                            $address = $party->litigant->addresses->first();
                                                                        @endphp
                                    ...
                                    <div class="col-md-6 mb-3">
                                        <div class="card bg-light border-0">
                                            <div class="card-body py-2">
                                                <div class="d-flex align-items-start">
                                                    <div
                                                        class="avatar-sm bg-success bg-opacity-10 rounded-circle me-3 d-flex align-items-center justify-content-center">
                                                        <i class="fas fa-home text-success"></i>
                                                    </div>
                                                    <div class="flex-grow-1">
                                                        <h6 class="mb-1">{{ ucfirst(str_replace('_', ' ', $asset->asset_type)) }}
                                                        </h6>
                                                        <div class="small text-muted">
                                                            ...
                                                            @elseif($asset->asset_type === 'movable_property_car' || $asset->asset_type === 'movable_property_motorcycle')
                                                                @if($asset->movableProperty)
                                                                    ...
                                                                    <div>Hiệu: {{ $asset->movableProperty->brand }}
                                                                        {{ $asset->movableProperty->model }}</div>
                ...
                <div class="card shadow-sm">
                    ...
                    <div class="card-body">
                        <div class="d-grid gap-2">
                            ...
                            <hr>
                            <a href="{{ route('admin.dossiers.show', $dossier) }}" class="btn btn-outline-primary">
                                <i class="fas fa-arrow-left me-2"></i>Về Hồ Sơ
                            </a>
                        </div>
                    </div>
